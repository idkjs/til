...
let () = {
  ...
  /* addは重複するkeyが存在する場合、戻り値として`Duplicateを返す。
     追加に成功した場合`Okを返す */
  let result = Hashtbl.add(table, "three", 3);
  assert(result == `Duplicate);
  /* add_exnは重複するkeyが存在する場合、例外を投げる */
  let _ =
    try(Hashtbl.add_exn(table, "three", 3)) {
  ...
  /* valueの存在確認 */
  assert(Hashtbl.exists(table, ~f=data => data == 1));
  /* keyとvalueの組み合わせの存在確認 */
  assert(
    Hashtbl.existsi(table, ~f=(~key, ~data) => key == "one" && data == 1),
  );
};
