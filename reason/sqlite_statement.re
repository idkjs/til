...
let () = {
  ...
  let db_statement = db_prepare(db, "SELECT * FROM sample_table");
  let column_count = Sqlite3.column_count(db_statement);
  let data_count = Sqlite3.data_count(db_statement);
  /* data count */
  printf("Data count\n");
  printf("%d\n", data_count);
  /* print column names */
  printf("Column names\n");
  let l = List.init(column_count, x => x);
  List.iter(l, n => Sqlite3.column_name(db_statement, n) |> printf("%S "));
  printf("\n");
};
